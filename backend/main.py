...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "http://localhost:5174",
        "http://localhost:5175",
    ],  # React dev server and Vite dev server
...
async def get_providers():
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to get providers"}
        )
...
async def get_available_models(provider: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        models = await api_manager.get_available_models(
            provider, api_key, db, security_manager, SettingsRepository
        )
...
async def get_settings(db: Session = Depends(get_db)):
    ...
    try:
        ...
        # Get model settings
        for setting in SettingsRepository.get_settings_by_category(
            db, "model_settings"
        ):
        ...
        return {"model_settings": model_settings, "app_settings": app_settings}
    except Exception as e:
        logger.error(f"Error getting settings: {e}")
        return JSONResponse(
            status_code=500, content={"error": "Failed to get settings"}
        )


@app.post("/api/settings")
async def update_settings(
    request: SettingsUpdateRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to update settings"}
        )
...
async def upload_video(
    ...
    try:
        await progress_manager.send_progress(project_id, "upload_started", 0)
        # Validate file type
        ...
        await progress_manager.send_progress(project_id, "file_saved", 50)

        # Process video metadata
        video_data = await video_processor.extract_metadata(file_path)
        await progress_manager.send_progress(project_id, "metadata_extracted", 90)
        ...
        await progress_manager.send_progress(project_id, "upload_completed", 100)
...
async def process_youtube(
    project_id: str, request: YouTubeURLRequest, db: Session = Depends(get_db)
):
    ...
    try:
        await progress_manager.send_progress(project_id, "youtube_started", 0)
        # Update project with the YouTube URL first
        ProjectRepository.update_project(
            db, project_id, {"youtube_url": request.youtube_url, "status": "processing"}
        )

        # Download and process YouTube video
        video_data = await video_processor.process_youtube_url(request.youtube_url)
        await progress_manager.send_progress(project_id, "youtube_downloaded", 70)

        # Update project
        updates = {"video_data": video_data, "status": "uploaded"}

        updated_project = ProjectRepository.update_project(db, project_id, updates)

        await progress_manager.send_progress(project_id, "youtube_completed", 100)
...
async def analyze_video(
    project_id: str, request: AnalysisPromptRequest, db: Session = Depends(get_db)
):
    ...
    try:
        await progress_manager.send_progress(project_id, "analysis_started", 0)
        # Update project with analysis prompt
        updates = {"analysis_prompt": request.prompt, "status": "analyzing"}
        updated_project = ProjectRepository.update_project(db, project_id, updates)

        # Get API settings
        provider_setting = SettingsRepository.get_setting(
            db, "model_settings", "default_provider"
        )
        provider = request.provider or (
            provider_setting.value if provider_setting else "openai"
        )
        ...
        async def progress_cb(p: int):
            await progress_manager.send_progress(project_id, "analysis", p)

        clips = await simulate_analysis(updated_project, request.prompt, progress_cb)
        ...
        await progress_manager.send_progress(project_id, "analysis_completed", 100)
...
async def simulate_analysis(
    project: Project, prompt: str, progress_cb=None
) -> List[Clip]:
    ...
    clip_types = {
        "funny": [
            ("Hilarious reaction", 85),
            ("Comedy gold moment", 92),
            ("Unexpected humor", 78),
        ],
        "engaging": [
            ("Hook moment", 88),
            ("Peak engagement", 94),
            ("Attention grabber", 82),
        ],
        "educational": [
            ("Key insight", 90),
            ("Learning moment", 87),
            ("Important concept", 85),
        ],
        "emotional": [
            ("Touching moment", 89),
            ("Emotional peak", 93),
    ...
    for i, (title, score) in enumerate(selected_clips):
        ...
        if progress_cb:
            progress = int(((i + 1) / len(selected_clips)) * 100)
...
async def update_clip(
    project_id: str, clip_id: str, clip_data: dict, db: Session = Depends(get_db)
):
    """Update a clip"""
    project = ProjectRepository.get_project_by_id(db, project_id)
    if not project:
...
async def export_clips(
    project_id: str, export_settings: dict, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        async def progress_cb(p: int):
            await progress_manager.send_progress(project_id, "export", p)
        ...
        await progress_manager.send_progress(project_id, "export_completed", 100)
...
async def set_api_key(request: APITestRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        validation_result = await api_manager.test_connection(
        ...
        return JSONResponse(
            status_code=500, content={"error": "Failed to store API key"}
        )
