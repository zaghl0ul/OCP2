    ...
    (set, get) => ({
      ...
      initialize: async () => {
        ...
        try {
          ...
          throw new Error(
            "Backend connection required. Please ensure the backend server is running.",
          );
        }
      },

      connectProgressSocket: (projectId) => {
        const wsBase = import.meta.env.VITE_API_WS_URL || "ws://localhost:8000";
        const socket = new WebSocket(`${wsBase}/ws/progress/${projectId}`);
        socket.onmessage = (event) => {
          try {
            const data = JSON.parse(event.data);
            set({
              processingProgress: data.progress || 0,
              processingStatus: data.event || "",
            });
          } catch (e) {
            console.error("WebSocket message error", e);
          }
        };
        socket.onclose = () => set({ progressSocket: null });
        set({ progressSocket: socket });
      },

      disconnectProgressSocket: () => {
        const { progressSocket } = get();
        if (progressSocket) {
          progressSocket.close();
        }
        set({ progressSocket: null });
      },
      ...
      addVideoToProject: async (projectId, videoData) => {
        ...
        get().connectProgressSocket(projectId);
        ...
        set((state) => ({
          ...
          currentProject:
            state.currentProject?.id === projectId
              ? {
                  ...state.currentProject,
                  clips: state.currentProject.clips.filter(
                    (clip) => clip.id !== clipId,
                  ),
                  updatedAt: Date.now(),
                }
        ...
        return projects
          .sort((a, b) => {
            ...
            const aDate = new Date(
              a.updated_at || a.updatedAt || a.created_at || 0,
            );
            const bDate = new Date(
              b.updated_at || b.updatedAt || b.created_at || 0,
            );
            return bDate - aDate;
          })
          .slice(0, limit);
      ...
      getProjectStats: () => {
        ...
        return {
          ...
          completedProjects: projects.filter((p) => p.status === "completed")
            .length,
          totalClips: projects.reduce(
            (sum, p) => sum + (Array.isArray(p.clips) ? p.clips.length : 0),
            0,
          ),
          averageScore:
            projects.length > 0
              ? projects.reduce((sum, p) => {
                  const projectAvg =
                    Array.isArray(p.clips) && p.clips.length > 0
                      ? p.clips.reduce((s, c) => s + (c?.score || 0), 0) /
                        p.clips.length
                      : 0;
